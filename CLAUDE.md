...
```bash
docker compose -f docker-compose.yml -f docker-compose.override.development.yml exec web КОМАНДА
# Примеры:
...
cp .env.example .env
# Заполнить .env
docker compose -f docker-compose.yml -f docker-compose.override.development.yml up -d
